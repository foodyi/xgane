...
#XGSDK_LIB="$PROJECT_DIR"/libXGSDK.a

echo "$XGSDK_LIB"

cp -f "$PROJECT_DIR"/libXGSDK.a "$CONFIGURATION_BUILD_DIR"
...
"$AIR_SDK_PATH"/bin/adt -package -target ane "$TARGET_NAME" extension.xml -swc "$EXTENSION_SWC_FILE_NAME" -platform default library.swf -platform iPhone-ARM -platformoptions platformoptions.xml libXGSDK.a "$NATIVE_EXTENSION_STATIC_LIB_NAME" library.swf
popd
